...
export function EnhancedInterestCard({ interest }: { interest: EnhancedInterest }) {
  ...
  return (
    <div className={cn("border rounded-sm overflow-hidden transition-colors", tierColor, open && "shadow-card")}>
      ...
      {open && (
        <div className="border-t border-border px-4 pb-4 space-y-4 bg-[#F8FAFC]/60">
...
export function EnhancedBehaviorCard({ behavior }: { behavior: EnhancedBehavior }) {
  ...
  return (
    <div className="border border-border rounded-sm overflow-hidden bg-[#F8FAFC] hover:border-border-strong transition-colors">
      <button type="button" onClick={() => setOpen(!open)} className="w-full text-left p-4">
        <div className="flex items-start justify-between gap-2 mb-2">
          <div className="flex-1 min-w-0">
    ...
    <div className="border border-border rounded-sm overflow-hidden bg-[#F8FAFC] hover:border-border-strong transition-colors">
      <button type="button" onClick={() => setOpen(!open)} className="w-full text-left p-4">
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-[10px] font-mono text-text-muted">{(demographic as unknown as Record<string,unknown>)["region"] as string ?? ""}</span>
        </div>
