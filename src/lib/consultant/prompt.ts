...
Relevant playbooks (real, structured consultant workflows selected for the actual problems present — use their likelyCauses and recommendedActions as a starting structure, but only cite a likely cause if its confirmingEvidence is actually present in the context above, and only apply a recommended action's confidenceRule honestly. If more than one playbook is present, address each real problem separately rather than blending them into one answer): ${JSON.stringify(context.playbooks)}
