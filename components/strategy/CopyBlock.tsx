...
export function CopyBlock({ label, text, mono, maxLength, className }: CopyBlockProps) {
  ...
  return (
    <div className={cn("relative group bg-[#0B1120] border border-border rounded-sm", className)}>
      {label && (
...
export function CopyChip({ text, label }: { text: string; label?: string }) {
  ...
  return (
    <button
      type="button"
      onClick={handleCopy}
      className="group flex items-center gap-2 px-3 py-2 bg-[#0B1120] border border-border rounded-sm hover:border-primary transition-colors text-left"
    >
